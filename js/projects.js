...
// NOTE: Placeholder images in assets/placeholders are intentionally retained because each project
// references a specific demo image (chatbot.jpg, pdf-chat.jpg, sentiment.jpg, placeholder.jpg).
// If you later add real thumbnails, replace the image paths below and then you can prune unused ones.
const PROJECTS = [
  {
    id: 'chat-with-pdf',
    title: 'Chat-with-PDF',
...
function populateFeatured(){
  ...
  PROJECTS.slice(0,6).forEach((p,idx) => {
    ...
    card.innerHTML = `
      ...
      <h3>${p.title}</h3>
      <p>${p.description}</p>
      <div class="project-tags" aria-label="Technologies used">${p.tags.map(t => `<span>${t}</span>`).join('')}</div>
      <div class="card-actions">
  ...
  // trigger animation class after next frame to ensure initial style application
  requestAnimationFrame(()=> {
...
function bindProjectModals(){
  document.addEventListener('click', (e) => {
    ...
    if(!project) return;
    const tagHTML = project.tags.map(t => `<span class="skill-pill" style="font-size:.58rem; font-weight:600; background:var(--color-bg-alt); border:1px solid var(--color-border); padding:.25rem .5rem; border-radius:var(--radius-full); text-transform:uppercase; letter-spacing:.6px;">${t}</span>`).join(' ');
    ...
    const actions = [];
      ...
      if(isGitHub){
        ...
        if(parts) ghSlug = parts.replace(/\.git$/,'');
      }
      // Only the four requested buttons: Repo, README, Download, Live
      if(repo) actions.push(`<a href="${repo}" target="_blank" rel="noopener" class="btn primary" aria-label="Open repository for ${project.title}">Repo</a>`);
      if(isGitHub) actions.push(`<a href="${repo}#readme" target="_blank" rel="noopener" class="btn outline" aria-label="Open README for ${project.title}">README</a>`);
      if(isGitHub) actions.push(`<a href="${repo}/archive/refs/heads/main.zip" target="_blank" rel="noopener" class="btn outline" aria-label="Download ZIP for ${project.title}">Download</a>`);
      if(project.live) actions.push(`<a href="${project.live}" target="_blank" rel="noopener" class="btn secondary" aria-label="Open live demo for ${project.title}">Live</a>`);
    const html = `
      <article class="project-detail" style="max-width:880px;">
        ...
        <div class="project-detail-body" style="display:grid; gap:1.2rem;">
          ...
          ${highlights ? `<section><h3 style="font-size:1rem; margin:.2rem 0 .5rem;">Highlights</h3><ul style="margin:0 0 .8rem 1.1rem; padding:0; list-style:disc; display:grid; gap:.35rem;">${highlights}</ul></section>`:''}
          <section>
