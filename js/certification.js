// certification.js - renders certification cards & modal (refactored for readability & idempotence)
(function () {
    'use strict';
    ...
    function providerLogoHTML(provider) {
        return `<span class="cert-provider-logo" aria-hidden="true">${PROVIDER_LOGOS[provider] || PROVIDER_LOGOS.generic
            }</span>`;
    }
    ...
    function buildModal(cert) {
        ...
        const badge = isInProgress(cert)
            ? '<span class="badge badge-progress" aria-label="In Progress">In&nbsp;Progress</span>'
            : '';
        return `
      <article class="cert-modal">
        ...
        ${cert.topics && cert.topics.length
                ? `<section class="cert-skills"><h4 class="cert-skills-heading">Skills / Things Learned</h4><ul class="cert-skills-list">${cert.topics
                    .map((t) => `<li>${t}</li>`)
                    .join('')}</ul></section>`
                : ''
            }
        <div class="cert-actions">${cert.credential
                ? `<a href="${cert.credential}" target="_blank" rel="noopener" class="btn primary btn-credential">View Credential</a>`
                : ''
            }<button class="btn outline" data-modal-close>Close</button></div>
      </article>`;
    }
    ...
    function buildCard(cert) {
        ...
        const issuedYear = cert.issued
            ? new Date(cert.issued).getFullYear()
            : /^[0-9]{4}$/.test(cert.year)
                ? cert.year
                : cert.year;
    ...
    function renderAll() {
        ...
        if (!grid) return;
        if (grid.__rendered) return; // idempotence guard
    ...
    function updateStatus(visible, total) {
        ...
        if (!s) return;
        if (total === 0) {
            s.textContent = 'No certifications available yet.';
            return;
        }
    ...
    function updateHeading(filterKey) {
        ...
        if (!h) return;
        const map = {
            all: 'All Certification Items',
            ai: 'AI / ML Certification Items',
            python: 'Python Certification Items',
            data: 'Data Certification Items',
            cloud: 'Cloud Certification Items',
            dev: 'Development Certification Items'
        };
        h.textContent = map[filterKey] || 'Certification';
    }
    ...
    function openCertModal(id) {
        ...
        if (cert && window.__portfolioModal) window.__portfolioModal.open(buildModal(cert));
    }
    ...
    function init() {
        ...
        if (!statusEl) {
            ...
            if (filterSection) filterSection.appendChild(statusEl);
            else grid.before(statusEl);
        }
        ...
        try {
            console.info(
                '[Certification] Rendered items:',
                SORTED.map((c) => ({ id: c.id, category: c.category }))
            );
        } catch (e) { }
        wireFilterButtons();
        wireCardClicks();
